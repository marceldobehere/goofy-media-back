...
CREATE TABLE `__new_notifications` (
	...
	FOREIGN KEY (`commentUuid`) REFERENCES `comments`(`uuid`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`commentResponseUuid`) REFERENCES `comments`(`uuid`) ON UPDATE no action ON DELETE no action
);
